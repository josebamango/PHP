...
$location = "http://aplicaciones.ivanlorenzo.es/dwes/conversor/ConversorWSDL.php";
$objeto = new SoapClient($location);
$ratioPeso = 1;
$pesoString = "Kg";
if (isset($_GET["en"])) {
    $ratioPeso = $objeto->cambiarPeso("kg", "libras");
    $pesoString = "Libras";
}

if (isset($_POST["botonMostrar-Jugadores"]) || isset($_POST["botonMostrar-Traspaso"]) || isset($_POST["actualizarPeso"])
    || isset($_GET["es"]) || isset($_GET["en"])) {
    $equipoSelected = $_REQUEST["equipo"];
...
function crearTablaJugadores($arrayJugadores, $ratioPeso) {
    foreach ($arrayJugadores as $jugador) {
        echo "<tr scope='row'>";
        echo '<td>'.$jugador["nombre"].'</td>';
        echo '<td><input type="number" name="peso[]" value="'.(round($jugador["peso"]*$ratioPeso)).'"></td>';
        echo '</tr>';
    }
}
...
<body class="container">
    ...
    <?php if (isset($_POST["botonMostrar-Jugadores"]) || isset($_GET["es"]) || isset($_GET["en"])) : ?>
        <form action="" method="get">
            <input type="hidden" name="equipo" value="<?= $equipoSelected ?>">
            <input type="submit" name="es" value="ES">
        </form>
        <form action="" method="get">
            <input type="hidden" name="equipo" value="<?= $equipoSelected ?>">
        ...
        <form method="POST" action="<?= htmlspecialchars($_SERVER["PHP_SELF"]) ?>">
            ...
            <table class="table border mt-3">
                <tr>
                    <th>Nombre</th>
                    <th>Peso <?= $pesoString ?></th>
                </tr>
                <?php crearTablaJugadores($jugadoresEquipoSelected, $ratioPeso) ?>
            </table>
